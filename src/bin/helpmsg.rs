...
fn get_bins() -> Vec<String> {
	std::fs::read_dir("src/bin")
		...
		.map(|f| f.file_stem().unwrap().to_owned().into_string().expect("Expected example name to containe valid utf-8 chars"))
		.collect()
}

fn main() {
    println!("\n\nWelcome to the rust examples library!");
    println!("Contained within this repo are many examples of how simple tasks could be achived in rust");
